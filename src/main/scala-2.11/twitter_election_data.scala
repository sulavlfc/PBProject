...
object Util_election {
  ...
  val fw = new FileWriter("election.json",true)
...
object StatusStreamer_election {
  def main(args: Array[String]) {
    val twitterStream = new TwitterStreamFactory(Util_election.config).getInstance
    twitterStream.addListener(Util_election.simpleStatusListener)
    //val keyword = Array("elclasico","elclassico","el clasico","el classico")
    val keyword = Array("election","elections")
